            ...
            <div className="text-center">
                {!isHovered ? (
                    ...
                    <>
                        ...
                        <div className="text-lg text-purple-600 font-medium">{kanji.hanviet}</div>
                        <div className="flex justify-center gap-2 mb-3">
                            <Tag color={getJlptColor(kanji.jlpt_level)}>{kanji.jlpt_level}</Tag>
                            <Tag color="geekblue">{kanji.stroke_count} nét</Tag>
                        </div>
                        <div className="text-sm text-gray-500">Click để xem chi tiết</div>
                    </>
                ) : (
                    // Back content
                    <div className="flex flex-col justify-center items-center h-full w-full">
                        <div className="text-6xl font-bold text-gray-800">{kanji.character}</div>
                        <div className="text-xl font-semibold text-gray-800 mt-2">{kanji.meaning_vi}</div>
                    </div>
                )}
            </div>
        </Card>
